#!/usr/bin/env python3
"""
ICTC VLM Clustering — Track 2: Algorithmic Identity & Profiling
===================================================================
Single-GPU shard script (Unified Qwen 3.5) — TRACK 2 PROMPTS

Researches the "Data Double" — what kind of demographic buckets the
algorithm places individuals into based on the ads they are shown.
...
Usage:
  python ictc_cluster_single_gpu_track2.py \
    --ads_dir /data/ads --output_dir /data/out --shard_id track2_identity
...
# ---------------------------------------------------------------------------
# PROMPTS  (Track 2: Algorithmic Identity & Profiling)
# ---------------------------------------------------------------------------
...
### STEP 2: DESCRIBE
- If "ADVERTISEMENT": Extract brand_name, main_text, and a visual_summary that analyzes the "Assumed User Identity." Detail the socio-economic status, gender performance, cultural signals, or lifestyle the ad assumes the viewer has or wants. Who does the algorithm think is watching this?
...
STEP2B_TEMPLATE = """You are a Critical Data Scholar researching surveillance capitalism and algorithmic identity profiling.
...
STEP3_SYSTEM_TEMPLATE = """You are classifying targeted ads by the demographic identity the algorithm has assigned to the user.
...
def run_step2b(
    ...
    response = llm.generate_single(
        "You are a Critical Data Scholar researching surveillance capitalism and algorithmic identity profiling.", user_msg, max_tokens=800
    )
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="ICTC VLM Clustering — Track 2: Algorithmic Identity & Profiling (unified Qwen 3.5)",
    ...
    p.add_argument("--criterion", type=str, default="Algorithmic Identity Profiling",
...
def main() -> None:
    ...
    log.info("=" * 70)
    log.info("ICTC Track 2: Algorithmic Identity — Single-GPU Run")
